...
def call_editor(
    ...
    try:
        ...
        logging.warning(f"Error calling LLM in step5: {e}")
        return None
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=("Step5: edit split2 and generate edited main caption in metadata.")
    )
    parser.add_argument("-i", "--input_jsonl", required=True, help="Path to input metadata JSONL")
    parser.add_argument(
        "-o", "--output_jsonl", required=True, help="Path to output metadata.step5_edit_merge.jsonl"
    )
    parser.add_argument(
    ...
    if args.config_path:
        args, config = apply_step_config(args, "step5_edit_split2_merge_main")
        step_cfg = config["step5_edit_split2_merge_main"]
